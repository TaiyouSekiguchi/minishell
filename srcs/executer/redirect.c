#include "minishell.h"

void	redirect_file_open(char *file_name, char **my_env, int *infile_fd, int *outfile_fd)
{
	if (file_name[0] == '<' && file_name[1] == ' ')
		*infile_fd = infile_open(file_name);
	else if (file_name[0] == '<' && file_name[1] == '<')
		*infile_fd = heredoc_open(file_name, my_env);
	else if (file_name[0] == '>' && file_name[1] == ' ')
		*outfile_fd = outfile_open(file_name);
	else if (file_name[0] == '>' && file_name[1] == '>')
		*outfile_fd = append_open(file_name);
	else
	{
		*infile_fd = ERROR_FD;
		*outfile_fd = ERROR_FD;
	}
}

void	get_redirect_fd(t_list *redirect, char **my_env, int *infile_fd, int *outfile_fd)
{
	char	*file_name;
	t_list	*current;

	*infile_fd = NONE_FD;
	*outfile_fd = NONE_FD;

	if (redirect == NULL)
		return ;

	file_name = NULL;
	current = redirect;
	while (current != NULL)
	{
		file_name = current->content;
		redirect_file_open(file_name, my_env, infile_fd, outfile_fd);
		if (*infile_fd == ERROR_FD || *outfile_fd == ERROR_FD)
			break ;
		current = current->next;
	}
}

/*int	get_redirect_fd(t_list *file_list, char **my_env)
{
	int		fd;
	char	*file_name;

	if (file_list == NULL)
		return (NONE_FD);

	file_name = NULL;
	while (file_list != NULL)
	{
		file_name = file_list->content;
		fd = redirect_file_open(file_name, my_env);
		if (fd == ERROR_FD)
			break ;
		file_list = file_list->next;
	}

	return (fd);
}*/

void	do_redirect(int infile_fd, int outfile_fd)
{
	if (infile_fd != NONE_FD)
	{
		close(STDIN);
		dup2(infile_fd, STDIN);
		close(infile_fd);
	}
	if (outfile_fd != NONE_FD)
	{
		close(STDOUT);
		dup2(outfile_fd, STDOUT);
		close(outfile_fd);
	}
}
